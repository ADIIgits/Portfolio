  ...
  return (
    <section id="education" className="py-32 px-4 sm:px-8 lg:px-16">
      <div className="max-w-4xl mx-auto">
        ...
        >
          <span className="text-foreground/20 font-mono text-xl sm:text-2xl">05.</span>
          Education
        </motion.h2>

        <div className="relative border-l border-foreground/10 ml-4 sm:ml-0">
          {education.map((edu, idx) => (
            ...
            >
              ...
              <div className="glass p-8 sm:p-10 rounded-[2rem] hover:border-foreground/20 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      {edu.location && (
                        <p className="flex items-center gap-1.5 text-sm text-foreground/40 mt-1">
                          <MapPin size={12} />
                          {edu.location}
                        </p>
                      )}
                  ...
                  <div className="flex flex-col items-start sm:items-end gap-1 shrink-0">
                    <span className="text-sm font-mono text-foreground/40 tracking-wider">
                      {edu.start} — {edu.end}
                    </span>
                    {edu.cgpa && (
                      <span className="text-sm font-semibold text-accent">CGPA: {edu.cgpa}</span>
                    )}
                    {edu.percentage && (
                ...
                {edu.highlights && edu.highlights.length > 0 && (
                  <div className="mt-6 space-y-2 border-t border-foreground/5 pt-6">
                    {edu.highlights.map((hl, i) => (
                      <p key={i} className="flex gap-3 text-foreground/60 font-light text-sm leading-relaxed">
